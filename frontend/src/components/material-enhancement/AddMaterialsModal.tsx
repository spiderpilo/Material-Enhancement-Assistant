...
export type AddMaterialsSubmitResult = {
  success: boolean;
  errorMessage?: string;
};
...
export function AddMaterialsModal({
  ...
  const handlePrimaryAction = async () => {
    ...
    const result = await Promise.resolve(onAddMaterials({ files: pendingFiles }))
      .catch((cause) => ({
        success: false,
        errorMessage:
    ...
    >
      <div className="flex w-full max-w-[908px] flex-col items-center gap-5">
        <section
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="add-materials-modal-title"
          aria-describedby="add-materials-modal-subtitle"
        ...
        <div className="flex items-center gap-2 text-[12px] text-[rgba(120,113,108,0.94)]">
